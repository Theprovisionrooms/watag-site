...
//
// Privacy / cookies / GDPR page, sits behind the artist login. Content
// below is a real first draft based on what this app actually collects
// and stores (see schema.sql), not filler text. Get it checked by a
// solicitor before treating it as final, but it's accurate to the code
// as it stands and safe to use as a working draft or to link to.
...
const SECTIONS = [
  ...
  {
    title: "Client rights (UK GDPR)",
    body: "Every client can ask to see what's held on them, correct anything wrong, or have their account deleted entirely. Send requests to [studio email] and treat them within one month. If a client asks for deletion, remove their row from clients along with anything that references it (loyalty card, messages, waitlist entries, referrals) unless there's a legal reason to keep it, e.g. a completed order for tax records.",
  },
  {
    title: "Data controller",
    body: "WATAG is the data controller for everything collected through this app. [Add registered business name, address and contact email here for the published version.]",
  },
];

export default function StaffLegal() {
  return (
    <div className="watag-screen">
      <NavBack to="/staff/home" label="home" />
      <h1>Privacy & data</h1>
      <p style={{ color: "var(--watag-text-dim)" }}>
        Staff reference for GDPR, cookies and privacy policy purposes. This is a working draft grounded in what the
        app actually stores, get it signed off before publishing anywhere client-facing.
      </p>
