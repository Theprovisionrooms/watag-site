...
//
// POST /api/staff/change-pin
// Body: { staffId, currentPin, newPin }
...
export async function onRequestPost({ request, env }) {
  const { staffId, currentPin, newPin } = await request.json();

  if (!staffId || !currentPin || !newPin) {
    return new Response(JSON.stringify({ error: "staffId, currentPin and newPin are required" }), {
      status: 400,
